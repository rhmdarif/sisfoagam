                                ...
                                <tbody>
                                    @foreach ($fasilitas as $i => $item)
                                        <tr>
                                            ...
                                            <td><img src="{{ storage_url($item->icon_fasilitas_akomodasi) }}" alt="{{ $item->nama_fasilitas_akomodasi }}" class="img-fluid" width="60px"> </td>
                                            <td>
    ...
    <script>
        $(document).ready(() => {
            ...
            $('#edit-fasilitas form').submit((e) => {
                ...
                $.ajax({
                    url: "{{ route('admin.master-data.akomodasi.fasilitas.index') }}/"+$('#edit-fasilitas #id').val(),
                    enctype: 'multipart/form-data',
                    type: "POST",
                    data: data,
                    processData: false,
        ...
        {
            $.ajax({
                url:'{{route("admin.master-data.akomodasi.fasilitas.index")}}/'+id,
                type:'get',
                dataType: 'json',
                success: function(data) {
                    console.log(data)
                    $('#edit-fasilitas #nama_fasilitas').val(data.nama_fasilitas_akomodasi)
                    $('#edit-fasilitas #tampilFoto').html(`<img src="{{ url('storage') }}/${data.icon_fasilitas_akomodasi}" width="30%"/>`)
