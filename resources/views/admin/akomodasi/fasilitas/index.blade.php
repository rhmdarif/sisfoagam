                                ...
                                <tbody>
                                    @foreach ($fasilitas as $i => $item)
                                        <tr>
                                            ...
                                            <td><img src="{{ storage_url("fasilitas_wisata", $item->icon_fasilitas_wisata) }}" alt="{{ $item->nama_fasilitas_wisata }}" class="img-fluid" width="60px"> </td>
                                            <td>
    ...
    <script>
        $(document).ready(() => {
            ...
            $('#edit-fasilitas form').submit((e) => {
                ...
                $.ajax({
                    url: "{{ route('admin.master-data.destinasi-wisata.fasilitas.index') }}/"+$('#id').val(),
                    enctype: 'multipart/form-data',
                    type: "POST",
                    data: data,
                    processData: false,
        ...
        function edits(id)
        {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#edit-fasilitas #tampilFoto').html(`<img src="{{ url('storage/fasilitas_wisata') }}/${data.icon_fasilitas_wisata}" width="30%"/>`)
